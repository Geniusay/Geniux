...
#define BIT_STAT_DRDY	  	  0X40		//驱动器准备完毕
#define BIT_STAT_DRQ 	  	  0x8		//数据传输准备完毕
...
void ide_read(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt)
{
	...
	{
		if ((secs_done + 256) <= sec_cnt)    secs_op = 256; //8位数寄存器每次最多读取255个扇区
...
void intr_hd_handler(uint8_t irq_no)
{
	...
	uint8_t ch_no = irq_no - 0x20 - 0xe;	//得到通道的索引值
...
{
	/*
	此处动态申请内存是有原因的，由于是递归函数，如果使用局部的内存例如 bs_info[512]，就会导致函数还没
	结束，有调用下一个函数，而局部变量便会堆积到栈中，我们的栈只有4096字节大小，这样只够我们最多递归6次
	所以这里我们要使用动态内存分配
	*/
	struct boot_sector* bs = sys_malloc(sizeof(struct boot_sector));

	ide_read(hd, ext_lba, bs, 1);	//读取一扇区的数据到hd中
	uint8_t part_idx = 0;
